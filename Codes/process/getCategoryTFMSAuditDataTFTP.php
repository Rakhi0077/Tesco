<?php
	...
	if (isset($_GET['Supplier_Number']) && isset($_GET['Category_Director']) && isset($_GET['userId'])){
		...
		$query="SELECT  distinct 
						...
						Country,
						Score
				FROM SQL_Server_TFMS_Audit_Data_Corrected_TFTP AS A
				left join Suppliers_List_Category_Tab as sl
				on A.Supplier_Number_Corrected=sl.Supplier_Number
				INNER JOIN (SELECT DISTINCT Supplier_Number,Category_Director,Category_Area,Commercial_Director,Commercial_Area FROM Suppliers_List_Category_Tab WHERE Active_Yes_No='YES') AS B

				ON A.Supplier_Number_Corrected=B.Supplier_Number AND A.Category_Director_Corrected=B.Category_Director and A.Commercial_Director_Corrected=B.Commercial_Director
