...
FROM DXWI_PROD_ROI_VIEW_ACCESS.VWI0WKS_WKLY_STORE_SALES_TPNB AS wks
JOIN DXWI_PROD_ROI_VIEW_ACCESS.VWI0BPR_BASE_PRODUCT AS bpr ON wks.Base_Product_Number = bpr.Base_Product_Number
JOIN DXWI_PROD_ROI_VIEW_ACCESS. VWI0BUY_DAILY_BUYER_LOOKUP AS buy ON bpr.Product_Sub_Group_Code = buy.Product_Sub_Group_Code
...
FROM DXWI_PROD_ROI_VIEW_ACCESS.VWI0BBF_DAILY_STR_TPNB_REFUNDS AS bbf
JOIN DXWI_PROD_ROI_VIEW_ACCESS.VWI0BPR_BASE_PRODUCT AS bpr ON bbf.Base_Product_Number = bpr.Base_Product_Number
JOIN DXWI_PROD_ROI_VIEW_ACCESS.VWI0BUY_DAILY_BUYER_LOOKUP AS buy ON bpr.Product_Sub_Group_Code = buy.Product_Sub_Group_Code
JOIN DXWI_PROD_ROI_VIEW_ACCESS.VWI0CAL_CALENDAR AS cal ON bbf.Calendar_Date = cal.Calendar_Date
WHERE bbf.Refund_Code IN ('1','2','3')
and bbf.calendar_date in (sel calendar_date from VWI0CAL_CALENDAR where year_week_number  between 201849 and 201850 )
--AND (Commercial_Director IN ('FRES          H.', 'PACKAGE       D.', 'TOBACCO & PF  S.') or buy. Product_Sub_Group_Code = 'H37PX')
GROUP BY 1,2
) B
...
WHERE wks.Year_Week_Number between 201849 and 201850
--AND (Commercial_Director  IN ('FRES          H.', 'PACKAGE       D.', 'TOBACCO & PF  S.') or buy. Product_Sub_Group_Code = 'H37PX')
