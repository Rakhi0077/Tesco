<?php
	...
	if (isset($_GET['userId']) && isset($_GET['CD']) && isset($_GET['QVIS']) && isset($_GET['rating']) && isset($_GET['overallrating']) && isset($_GET['category']) && isset($_GET['tesco_brand_ind'])){
		...
		if($category=='Grocery'){
			$sub_query_8=" A.Commercial_Area='Packaged' ";
		}
		if($category=='Prepared'){
			$sub_query_8=" A.Commercial_Area='Fresh' ";
		}
		// echo $access_level;
		// echo $category;
		...
		*/
		...
		if($CD=="All")
			$sub_query_8=" A.Commercial_Area LIKE '%%' ";
		else
			$sub_query_8=" A.Commercial_Area='$CD' ";
		...
		*/
		...
		$req_columns="B.Parent_Supplier_Name as Parent_Supplier_Name,B.Parent_Supplier_Number_TFMS,A.Commercial_Area,A.Category_Area,A.Own_Brand_Lines_Count,A.Brand_Lines_Count,Quality_Rank,Value_Rank,Innovation_Rank,Supply_Rank,QVIS_Rank";

		
		if ($QVIS=="All" AND $rating=="All")
			$query="SELECT distinct $req_columns FROM View_Rank_QVIS_Category AS A INNER JOIN (SELECT DISTINCT Parent_Supplier_Name,Parent_Supplier_Number_TFMS,Commercial_Area,Category_Area FROM Suppliers_List) AS B ON A.Supplier_Number=B.Parent_Supplier_Number_TFMS AND A.Category_Area=B.Category_Area WHERE $sub_query_3 AND $sub_query_4 AND $sub_query_5 AND $sub_query_6  AND $sub_query_7 AND $sub_query_8 ORDER BY A.Category_Area,QVIS_Rank DESC,Parent_Supplier_Name;";
			
		if ($QVIS=="All" AND $rating!="All")
			$query="SELECT distinct $req_columns FROM View_Rank_QVIS_Category AS A INNER JOIN (SELECT DISTINCT Parent_Supplier_Name,Parent_Supplier_Number_TFMS,Commercial_Area,Category_Area FROM Suppliers_List) AS B ON A.Supplier_Number=B.Parent_Supplier_Number_TFMS AND A.Category_Area=B.Category_Area WHERE $sub_query_3 AND $sub_query_4 AND Quality_Rank=$rating AND Value_Rank=$rating AND Supply_Rank=$rating AND $sub_query_5 AND $sub_query_6 AND $sub_query_7  AND $sub_query_8 ORDER BY A.Category_Area,QVIS_Rank DESC,Parent_Supplier_Name;";
				 
		if ($QVIS!="All" AND $rating=="All")
			$query="SELECT distinct $req_columns FROM View_Rank_QVIS_Category AS A INNER JOIN (SELECT DISTINCT Parent_Supplier_Name,Parent_Supplier_Number_TFMS,Commercial_Area,Category_Area FROM Suppliers_List) AS B ON A.Supplier_Number=B.Parent_Supplier_Number_TFMS AND A.Category_Area=B.Category_Area WHERE $sub_query_3 AND $sub_query_4 AND $sub_query_5 AND $sub_query_6 AND $sub_query_7  AND $sub_query_8 ORDER BY A.Category_Area,QVIS_Rank DESC,Parent_Supplier_Name;";
					
		if ($QVIS!="All" AND $rating!="All")
			$query="SELECT distinct $req_columns FROM View_Rank_QVIS_Category AS A INNER JOIN (SELECT DISTINCT Parent_Supplier_Name,Parent_Supplier_Number_TFMS,Commercial_Area,Category_Area FROM Suppliers_List) AS B ON A.Supplier_Number=B.Parent_Supplier_Number_TFMS AND A.Category_Area=B.Category_Area WHERE $sub_query_3 AND $sub_query_4 AND $QVIS=$rating AND $sub_query_5 AND $sub_query_6 AND $sub_query_7  AND $sub_query_8 ORDER BY A.Category_Area,QVIS_Rank DESC,Parent_Supplier_Name;";
