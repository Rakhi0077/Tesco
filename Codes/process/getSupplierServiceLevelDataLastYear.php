<?php
	...
	if (isset($_GET['Supplier_Number']) && isset($_GET['Category_Director']) && isset($_GET['Junior_Buyer']) && isset($_GET['userId'])){
		...
		$query="SELECT
			...
			FROM (SELECT DISTINCT Year_week_number FROM Tesco_Calendar WHERE Calendar_Date BETWEEN CONVERT(date, GETDATE() - 737) AND CONVERT(date, GETDATE() - 379)) AS A 
			LEFT JOIN 
			(SELECT A.Supplier_Number,
					Year_Week_Number,
					SL_Percentage FROM View_104_Weeks_Service_Level_Data AS A
